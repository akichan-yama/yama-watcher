...
yama-watcher/
  ...
  ikedakan/          富士山 池田館
    watch_ikedakan.py
    config.yaml      ← 監視したい日付・部屋タイプをここで指定（最大3件）
...
3. 「New repository secret」で、以下を1つずつ登録
   | Name | Value |
   |---|---|
   | `GMAIL_ADDRESS` | あき様のGmailアドレス（送信元） |
   | `GMAIL_APP_PASSWORD` | Gmailのアプリパスワード（16桁） |
   | `TO_ADDRESS` | 通知を受け取りたいメールアドレス |
...
target_rooms:
  ...
  - "3人部屋"
```
※池田館は他の3サイトと違い、サイトの裏側のAPIから取得した予約データをもとに
　「定員 − 予約人数（または部屋数）」を自動計算して空室判定しています。
　サイトの内部ロジックを推測で再現しているため、実際の表示とズレる可能性があります。
　`--debug` 実行時に `ikedakan/debug_data.json` が保存されるので、
　空きが出た際は必ず予約ページ本体でも確認してください。
...
3. 各フォルダで
   ...
   python watch_ikedakan.py --debug
   ```
